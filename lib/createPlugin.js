            ...
            if (Object.keys(others).indexOf(name) !== -1) {
              ...
              for (var key in others) {
                ...
                if (key.indexOf(name) === 0) {
                  req.locals.query.$or.push(_defineProperty({}, key, others[key]));
                }
              }
            }
          }
    ...
    handlers: {
      list: function list(Model, _ref7) {
        ...
        model.count(query).exec(function (err, total) {
          ...
          model.find(query).sort(sort).skip(offset).limit(limit).select(select).lean().exec(function (error, data) {
      ...
      retrieve: function retrieve(Models, _ref9) {
        ...
        Models[namespace].findOne({ _id: id }).select(select).exec(function (err, object) {
          if (err) return next(err);
      ...
      update: function update(Models, _ref10) {
        ...
        Model.findOne({ _id: id }).exec(function (err, object) {
          if (err) return next(err);
      ...
      delete: function _delete(Models, _ref11) {
        ...
        return Models[namespace].remove({ _id: id }).exec(function (error) {
