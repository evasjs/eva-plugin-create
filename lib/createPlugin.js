          ...
          if (['createdAt', 'updatedAt'].indexOf(name) !== -1 || paths[name].query) {
            ...
            if (Object.keys(others).indexOf(name) !== -1) {
              ...
              if (range.length === 2 && !!range[1]) {
                ...
                req.locals.query.$and.push(_defineProperty({}, name, { $in: others[name] }));
              ...
              for (var key in others) {
                ...
                if (key.indexOf(name) === 0) {
                  var v = others[key];
                  if (Array.isArray(v)) {
                    req.locals.query.$and.push(_defineProperty({}, key, { $in: v }));
                  } else {
                    if (['true', 'false', ''].includes(v)) {
                      req.locals.query.$and.push(_defineProperty({}, key, ['true', ''].includes(v) ? true : false));
                    } else {
                      req.locals.query.$and.push(_defineProperty({}, key, v));
                    }
                  }
                }
              }
