  ...
  return {
    ...
    middlewares: {
      ...
      'format/search': function formatSearch(Models, _ref6) {
        ...
        req.locals.query.$or = [];
        [Object.keys(paths), 'createdAt', 'updatedAt'].forEach(function (name) {
          if (['createdAt', 'updatedAt'].indexOf(name) !== -1 || paths[name].query) {
            // fuzzy search
            if (kw) {
              req.locals.query.$or.push(_defineProperty({}, name, new RegExp(kw, 'igm')));
            }
            ...
            if (Object.keys(others).indexOf(name) !== -1) {
              ...
              if (range.length === 2 && !!range[1]) {
                var gte = new Date(range[0]),
                    lt = new Date(range[1]);

                // 1.1 date range

                if (gte.getFullYear() > 1024 & lt.getFullYear() > 1024) {
                  req.locals.query.$or.push(_defineProperty({}, name, { $gte: gte, $lt: lt }));
                }{
                  req.locals.query.$or.push(_defineProperty({}, name, { $gte: range[0], $lt: range[1] }));
                }
              } else if (Array.isArray(others[name])) {
                req.locals.query.$or.push(_defineProperty({}, name, { $in: others[name] }));
              } else if (['true', 'false', ''].includes(others[name])) {
                req.locals.query.$or.push(_defineProperty({}, name, ['true', ''].includes(others[name]) ? true : false));
              } else {
                req.locals.query.$or.push(_defineProperty({}, name, others[name]));
              }
            } else {
              // @TODO
              for (var key in others) {
                ...
                if (key.indexOf(name) === 0) {
                  var v = others[key];
                  if (Array.isArray(v)) {
                    req.locals.query.$or.push(_defineProperty({}, key, { $in: v }));
                  } else {
                    if (['true', 'false', ''].includes(v)) {
                      req.locals.query.$or.push(_defineProperty({}, key, ['true', ''].includes(v) ? true : false));
                    } else {
                      req.locals.query.$or.push(_defineProperty({}, key, v));
                    }
                  }
                }
              }
